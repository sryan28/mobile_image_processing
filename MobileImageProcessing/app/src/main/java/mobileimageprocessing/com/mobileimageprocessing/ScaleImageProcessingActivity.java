...
public class ScaleImageProcessingActivity extends BaseImageProcessingActivity {
    ...
    public int[][] processImageThreads(int[][] image, int threads) {
        //4 threads
        return resizeThreads(image, (int) Math.floor(image.length * 0.5), (int) Math.floor(image[0].length * 0.5), threads);
    }

    @Override
    public int[][] processImageLooper(int [][] image, int threads) {
        //4 threads pipelining
        return resizeLooper(image, (int) Math.floor(image.length * 0.5), (int) Math.floor(image[0].length * 0.5), threads);
    }
